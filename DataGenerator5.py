...
# Fonction pour créer un logo minier plus joli avec matplotlib
def generate_mining_logo():
    # Création d'une image stylisée de l'industrie minière
    ...
    # Contour d'une montagne dorée (représentant une mine)
    mountain_x = np.array([-1, -0.5, 0, 0.5, 1])
    mountain_y = np.array([0, 0.7, 0.3, 0.8, 0])
    ax.fill(mountain_x, mountain_y, color='#DAA520', alpha=0.8)
    ax.plot(mountain_x, mountain_y, color='#8B4513', linewidth=2)
    
    # Ajouter un "puits de mine"
    ax.add_patch(plt.Circle((0, 0.4), 0.15, color='#4B3621'))
    ax.add_patch(plt.Circle((0, 0.4), 0.10, color='#1A1110'))
    
    # Petite excavatrice stylisée
    # Corps de l'excavatrice
    ax.add_patch(plt.Rectangle((-0.3, -0.3), 0.6, 0.25, color='#FFD700'))
    
    # Roues
    ax.add_patch(plt.Circle((-0.15, -0.35), 0.1, color='#333333'))
    ax.add_patch(plt.Circle((0.15, -0.35), 0.1, color='#333333'))
    
    # Bras d'excavation
    ax.plot([-0.05, 0.4], [-0.15, 0.3], color='#8B4513', linewidth=3)
    ax.plot([0.4, 0.7], [0.3, 0.0], color='#8B4513', linewidth=3)
    
    # Godet
    godet_x = np.array([0.7, 0.85, 0.7])
    godet_y = np.array([0.0, -0.1, -0.2])
    ax.fill(godet_x, godet_y, color='#A0522D')
    
    # Ajout d'une touche métallique/brillante
    for i in range(5):
        y = mountain_y + np.random.uniform(-0.05, 0.05, len(mountain_y))
        ax.plot(mountain_x, y, color='#FFDF00', alpha=0.3, linewidth=1)
...
# Affichage du logo et du titre centré
st.markdown(
    """
    <style>
    .title-container {
...
st.markdown(
    ...
    <div class="title-container">
        <img class="logo-image" src="data:image/png;base64,{}">
    ...
    """.format(
        base64.b64encode(generate_mining_logo().getvalue()).decode()
    ),
    unsafe_allow_html=True
)
...
def generate_block_model(nx, ny, nz, block_size_x, block_size_y, block_size_z, 
    ...
    for i, metal in enumerate(selected_metals):
        ...
        final_values = combined_field_flattened * std_values[i] + mean_values[i]
